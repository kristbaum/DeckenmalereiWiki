...
class DeckenmalereiParser:
    ...
    def get_relations_by_type(self, entity_id: str, rel_type: str) -> List[Dict]:
        ...
        return [r for r in self.relations_by_source.get(entity_id, []) 
    ...
    def get_text_parts(self, text_entity_id: str) -> List[Dict]:
        ...
        part_relations = self.get_relations_by_type(text_entity_id, "PART")
        # Sort by relOrd
    ...
    def html_to_mediawiki(self, html: str) -> str:
        ...
        # Lists
        text = re.sub(r'<ul>(.*?)</ul>', lambda m: self._convert_list(m.group(1), '*'), text, flags=re.DOTALL)
        text = re.sub(r'<ol>(.*?)</ol>', lambda m: self._convert_list(m.group(1), '#'), text, flags=re.DOTALL)
        
        # Links (basic conversion)
        text = re.sub(r'<a\s+href=["\']([^"\']+)["\']>(.*?)</a>', r'[\1 \2]', text, flags=re.DOTALL)
    ...
    def generate_all_articles(self, max_articles: Optional[int] = None) -> Dict[str, str]:
    ...
    def save_articles_to_files(self, output_dir: str = "output", max_articles: Optional[int] = None):
